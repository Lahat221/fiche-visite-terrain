...
// fonction pour la photo de plan
...
async function exportToPDF() {
  ...
  // Gestion des photos des plans
  const photos = document.getElementById("photoPlans").files;
  if (photos && photos.length > 0) {
    await addPhotos("Photos des plans", photos);
  } else if (plansMaison === "non") {
    addField(
      "Photos des plans",
      "Aucune photo téléchargée malgré l'absence de plans."
    );
  } else {
    addField("Photos des plans", "Aucune photo téléchargée.");
  }
  ...
  // Section 4 : Informations sur la Façade Nord
  addSectionTitle("Façade Nord");

  // Orientation du mur
  addField(
    "Orientation du mur",
    document.getElementById("orientationMurNord").value
  );

  // Type de murs
  addField("Type de murs", document.getElementById("typeMurNord").value);

  // Photos de loin
  await addPhotos(
    "Photos de loin - Façade Nord",
    document.getElementById("photosLoinNord").files
  );

  // Mur mitoyen
  addField("Mur mitoyen ?", document.getElementById("murMitoyenNord").value);

  // Détails de la mitoyenneté (Toujours affichés)
  addField(
    "Détails de la mitoyenneté",
    document.getElementById("detailsMitoyenneteNord").value
  );

  // Dimensions du mur
  addField("Longueur du mur", document.getElementById("longueurMurNord").value);
  addField(
    "Hauteur sous gouttières",
    document.getElementById("hauteurSousGouttieresNord").value
  );
  addField(
    "Surface totale",
    document.getElementById("surfaceTotaleNord").value
  );

  // Isolation
  addField("Mur isolé ?", document.getElementById("murIsoleNord").value);
  addField("Type d'isolant", document.getElementById("typeIsolantNord").value);
  addField(
    "Épaisseur de l'isolant (mm)",
    document.getElementById("epaisseurIsolantNord").value
  );

  // Fenêtres
  const numberOfWindows = parseInt(
    document.getElementById("nombreFenetreFacadeNord").value
  );

  if (!isNaN(numberOfWindows) && numberOfWindows > 0) {
    for (let i = 1; i <= numberOfWindows; i++) {
      // Récupérer les informations des fenêtres
      const typeFenetre =
        document.getElementById(`typeFenetreNord${i}`)?.value || "Non spécifié";
      const typeVitrage =
        document.getElementById(`typeVitrageNord${i}`)?.value || "Non spécifié";
      const typeVolet =
        document.getElementById(`typeVoletNord${i}`)?.value || "Non spécifié";
      const dimensions =
        document.getElementById(`dimensionsFenetreNord${i}`)?.value ||
        "Non spécifié";
      const anneeMiseEnPlace =
        document.getElementById(`anneeMiseEnPlaceNord${i}`)?.value ||
        "Non spécifié";

      // Ajouter les informations des fenêtres
      addField(`Fenêtre ${i} - Type de Fenêtre`, typeFenetre);
      addField(`Fenêtre ${i} - Type de Vitrage`, typeVitrage);
      addField(`Fenêtre ${i} - Type de Volet`, typeVolet);
      addField(`Fenêtre ${i} - Dimensions (L x H en cm)`, dimensions);
      addField(`Fenêtre ${i} - Année de mise en place`, anneeMiseEnPlace);

      // Ajouter les photos des fenêtres
      const photosFenetre = document.getElementById(
        `photosFenetreNord${i}`
      )?.files;
      if (photosFenetre && photosFenetre.length > 0) {
        await addPhotos(`Photos de Fenêtre ${i} - Façade Nord`, photosFenetre);
      } else {
        addField(`Photos de Fenêtre ${i}`, "Aucune photo téléchargée");
      }
    }
  } else {
    // Aucun fenêtre définie
    addField("Fenêtres", "Aucune fenêtre définie");
  }

  // Année d'isolation
  addField(
    "Année d'isolation",
    document.getElementById("dateIsolationNord").value
  );

  // Photos croquis de façade
  await addPhotos(
    "Photos croquis de Façade Nord",
    document.getElementById("photosCroquisFacadeNord").files
  );

  // Section 4 : Informations sur la Façade Sud
  addSectionTitle("Façade Sud");

  // Orientation du mur
  addField(
    "Orientation du mur",
    document.getElementById("orientationMurSud").value
  );

  // Type de murs
  addField("Type de murs", document.getElementById("typeMurSud").value);

  // Photos de loin
  await addPhotos(
    "Photos de loin - Façade Sud",
    document.getElementById("photosLoinSud").files
  );

  // Mur mitoyen
  addField("Mur mitoyen ?", document.getElementById("murMitoyenSud").value);

  // Détails de la mitoyenneté (Toujours affichés)
  addField(
    "Détails de la mitoyenneté",
    document.getElementById("detailsMitoyenneteSud").value
  );

  // Dimensions du mur
  addField("Longueur du mur", document.getElementById("longueurMurSud").value);
  addField(
    "Hauteur sous gouttières",
    document.getElementById("hauteurSousGouttieresSud").value
  );
  addField("Surface totale", document.getElementById("surfaceTotaleSud").value);

  // Isolation
  addField("Mur isolé ?", document.getElementById("murIsoleSud").value);
  addField("Type d'isolant", document.getElementById("typeIsolantSud").value);
  addField(
    "Épaisseur de l'isolant (mm)",
    document.getElementById("epaisseurIsolantSud").value
  );
  ...
  if (!isNaN(numberOfWindows) && numberOfWindows > 0) {
    for (let i = 1; i <= numberOfWindows; i++) {
      // Récupérer les informations des fenêtres
      const typeFenetre =
        document.getElementById(`typeFenetreSud${i}`)?.value || "Non spécifié";
      const typeVitrage =
        document.getElementById(`typeVitrageSud${i}`)?.value || "Non spécifié";
      const typeVolet =
        document.getElementById(`typeVoletSud${i}`)?.value || "Non spécifié";
      const dimensions =
        document.getElementById(`dimensionsFenetreSud${i}`)?.value ||
        "Non spécifié";
      const anneeMiseEnPlace =
        document.getElementById(`anneeMiseEnPlaceSud${i}`)?.value ||
        "Non spécifié";
      ...
      if (photosFenetre && photosFenetre.length > 0) {
        await addPhotos(`Photos de Fenêtre ${i} - Façade Sud`, photosFenetre);
      } else {
        addField(`Photos de Fenêtre ${i}`, "Aucune photo téléchargée");
      }
    }
  ...
  addField(
    "Année d'isolation",
    document.getElementById("dateIsolationSud").value
  );

  // Photos croquis de façade
  await addPhotos(
    "Photos croquis de Façade Sud",
    document.getElementById("photosCroquisFacadeSud").files
  );

  // Section 4 : Informations sur la Façade Ouest
  addSectionTitle("Façade Ouest");

  // Orientation du mur
  addField(
    "Orientation du mur",
    document.getElementById("orientationMurOuest").value
  );

  // Type de murs
  addField("Type de murs", document.getElementById("typeMurOuest").value);

  // Photos de loin
  await addPhotos(
    "Photos de loin - Façade Ouest",
    document.getElementById("photosLoinOuest").files
  );

  // Mur mitoyen
  addField("Mur mitoyen ?", document.getElementById("murMitoyenOuest").value);

  // Détails de la mitoyenneté (Toujours affichés)
  addField(
    "Détails de la mitoyenneté",
    document.getElementById("detailsMitoyenneteOuest").value
  );

  // Dimensions du mur
  addField(
    "Longueur du mur",
    document.getElementById("longueurMurOuest").value
  );
  addField(
    "Hauteur sous gouttières",
    document.getElementById("hauteurSousGouttieresOuest").value
  );
  addField(
    "Surface totale",
    document.getElementById("surfaceTotaleOuest").value
  );

  // Isolation
  addField("Mur isolé ?", document.getElementById("murIsoleOuest").value);
  addField("Type d'isolant", document.getElementById("typeIsolantOuest").value);
  addField(
    "Épaisseur de l'isolant (mm)",
    document.getElementById("epaisseurIsolantOuest").value
  );
  ...
  if (!isNaN(numberOfWindows) && numberOfWindows > 0) {
    for (let i = 1; i <= numberOfWindows; i++) {
      // Récupérer les informations des fenêtres
      const typeFenetre =
        document.getElementById(`typeFenetreOuest${i}`)?.value ||
        "Non spécifié";
      const typeVitrage =
        document.getElementById(`typeVitrageOuest${i}`)?.value ||
        "Non spécifié";
      const typeVolet =
        document.getElementById(`typeVoletOuest${i}`)?.value || "Non spécifié";
      const dimensions =
        document.getElementById(`dimensionsFenetreOuest${i}`)?.value ||
        "Non spécifié";
      const anneeMiseEnPlace =
        document.getElementById(`anneeMiseEnPlaceOuest${i}`)?.value ||
        "Non spécifié";
      ...
      // Ajouter les photos des fenêtres
      const photosFenetre = document.getElementById(
        `photosFenetreOuest${i}`
      )?.files;
      if (photosFenetre && photosFenetre.length > 0) {
        await addPhotos(`Photos de Fenêtre ${i} - Façade Ouest`, photosFenetre);
      } else {
        addField(`Photos de Fenêtre ${i}`, "Aucune photo téléchargée");
      }
    }
  ...
  addField(
    "Année d'isolation",
    document.getElementById("dateIsolationOuest").value
  );

  // Photos croquis de façade
  await addPhotos(
    "Photos croquis de Façade Ouest",
    document.getElementById("photosCroquisFacadeOuest").files
  );
  // Section 4 : Informations sur la Façade Est
  addSectionTitle("Façade Est");

  // Orientation du mur
  addField(
    "Orientation du mur",
    document.getElementById("orientationMurEst").value
  );

  // Type de murs
  addField("Type de murs", document.getElementById("typeMurEst").value);

  // Photos de loin
  await addPhotos(
    "Photos de loin - Façade Est",
    document.getElementById("photosLoinEst").files
  );

  // Mur mitoyen
  addField("Mur mitoyen ?", document.getElementById("murMitoyenEst").value);

  // Détails de la mitoyenneté (Toujours affichés)
  addField(
    "Détails de la mitoyenneté",
    document.getElementById("detailsMitoyenneteEst").value
  );

  // Dimensions du mur
  addField("Longueur du mur", document.getElementById("longueurMurEst").value);
  addField(
    "Hauteur sous gouttières",
    document.getElementById("hauteurSousGouttieresEst").value
  );
  addField("Surface totale", document.getElementById("surfaceTotaleEst").value);

  // Isolation
  addField("Mur isolé ?", document.getElementById("murIsoleEst").value);
  addField("Type d'isolant", document.getElementById("typeIsolantEst").value);
  addField(
    "Épaisseur de l'isolant (mm)",
    document.getElementById("epaisseurIsolantEst").value
  );
  ...
  if (!isNaN(numberOfWindows) && numberOfWindows > 0) {
    for (let i = 1; i <= numberOfWindows; i++) {
      // Récupérer les informations des fenêtres
      const typeFenetre =
        document.getElementById(`typeFenetreEst${i}`)?.value || "Non spécifié";
      const typeVitrage =
        document.getElementById(`typeVitrageEst${i}`)?.value || "Non spécifié";
      const typeVolet =
        document.getElementById(`typeVoletEst${i}`)?.value || "Non spécifié";
      const dimensions =
        document.getElementById(`dimensionsFenetreEst${i}`)?.value ||
        "Non spécifié";
      const anneeMiseEnPlace =
        document.getElementById(`anneeMiseEnPlaceEst${i}`)?.value ||
        "Non spécifié";
      ...
      if (photosFenetre && photosFenetre.length > 0) {
        await addPhotos(`Photos de Fenêtre ${i} - Façade Est`, photosFenetre);
      } else {
        addField(`Photos de Fenêtre ${i}`, "Aucune photo téléchargée");
      }
    }
  ...
  addField(
    "Année d'isolation",
    document.getElementById("dateIsolationEst").value
  );

  // Photos croquis de façade
  await addPhotos(
    "Photos croquis de Façade Est",
    document.getElementById("photosCroquisFacadeEst").files
  );
  ...
  // Section 9 : Informations sur la Ventilation
  addSectionTitle("Informations sur la Ventilation");
  addField("Système de ventilation", formData.get("systemeVentilation"));
  addField("Année d'installation", formData.get("anneeVentilation"));
  addField(
    "Système de refroidissement",
    formData.get("systemeRefroidissement")
  );
  addField("Commentaire", formData.get("commentaireVentilation"));

  // Ajout des photos de ventilation
  const photosVentilation = document.getElementById("photosVentilation").files;
  await addPhotos("Photos de la ventilation", photosVentilation);
